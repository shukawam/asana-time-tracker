...
> **ソースから（コントリビュータ向け）**: `git clone https://github.com/shukawam/asana-time-tracker && cd asana-time-tracker && npm install && npm link`。開発時は `tsx` でTypeScriptを直接実行するためビルド不要です（[Development](#開発) 参照）。
