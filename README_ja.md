...
> **ソースから（コントリビュータ向け）**: `git clone https://github.com/shukawam/asana-time-tracker && cd asana-time-tracker && npm install && npm link`。`npm install` で `prepare` script が走り `dist/` をビルドするので、`npm link` 時点で実行ファイルが存在します。日々の開発ループは `tsx` で再ビルド不要（[開発](#開発)参照）。
...
2. 以下を実行:
   ...
   PATの入力（または環境変数 `ASANA_PAT` の利用）、CSVの **Kong Resource** 列に出す固定文字列（例: `Field Engineer`。空欄ならその列は空のまま）、その後 **顧客プロジェクトごとの短いalias** を対話的に登録します（例: `acme` → "ACME Corp"）。

設定ファイルは `~/.config/att/config.json` （ファイル mode 0600、ディレクトリ mode 0700）に保存されます。`ASANA_PAT` 環境変数を設定している場合、`att init` は設定ファイル側に残っていた古いPATを *削除* します（env優先の挙動を実際に成立させるため）。
...
`entry-gid` は `att list` の出力に表示されます。`att edit` / `att rm` は **自分のエントリ専用** で、他人のエントリのGIDを指定するとAPIアクセスがあっても `att` 側で拒否します（共有workspaceでのタイポ事故を防ぐためのガード）。他人のエントリを変更したい場合は Asana Web UI を使ってください。
...
Asanaの各time entryが1行になります。**`--customer <alias>` は必須** で、付けずに csv 出力しようとすると `att` 側でエラー終了します（顧客別シートに別顧客のhoursを誤計上するのを防ぐため）。0hに丸まる行は出力されません。日付は `YYYY/M/D`、時間は `Math.round(min/60)`、**Kong Resource は `config.kongResource` の固定文字列**（`att init` で設定。未設定なら空欄）が全行同じ値で入ります。
...
CSV の **Kong Resource** 列には `config.kongResource` の固定文字列（例: `Field Engineer`）が全行同じ値で入ります。`att init` で対話的に設定するか、`~/.config/att/config.json` を直接編集してください。未設定の場合は空欄です。

> 補足: 元々は per-entry に Asana 標準の **Time Tracking Category** を割り当てる設計でしたが、対象ワークスペースで Categories API が有効になっていなかったため、この per-config の固定値方式に変更しました。
...
```bash
att install-skill     # 同梱の SKILL.md → ~/.claude/skills/att/SKILL.md にsymlink
```

インストール後、Claude Code内で `/att` と打つか、単に作業内容を話しかけてください。例: 「今 1.5h ACME のキックオフやってた、記録して」。既にClaude Codeを起動中だった場合は再起動してください。アンインストールは `rm ~/.claude/skills/att/SKILL.md`。symlinkは導入済みパッケージを指すので、`npm update -g @shukawam/asana-time-tracker`（ソース導入なら `git pull`）でSkillも自動更新されます。
...
npm run att -- ...    # 開発時のCLI実行（tsx — ビルド不要）
npm run build         # dist/ にコンパイル（publish時は prepublishOnly で自動実行）
npm pack --dry-run    # publish対象のtarball内容を事前確認
```
