...
- User asks to add a customer → call `att projects add` directly. Removal (`att projects rm`) is destructive — see the "must NOT" list; defer to the user.
...
Cache these in memory for the rest of the session. If `att` isn't on PATH, tell the user to run `npm install -g @shukawam/asana-time-tracker`, or `npm link` from a local checkout.
...
```
About to log:
  1. 1.5h ACME Corp — "kickoff meeting"  date=today
  2. 0.5h Beta Inc — pick from recent     date=today
Proceed? [y/N]
```
...
| 「Beta に 2h 設計レビュー」 | `att log 2 beta "設計レビュー"` |
...
- Never call `att rm`, `att edit`, `att projects rm`, or any other destructive op — even if the user asks. Defer to direct Bash invocation by the user (or by Claude outside this skill).
- Never auto-create customer aliases. Always defer to `att projects add`.
